...
train_df = train_df.drop(['sunrise', 'sunset', 'date','dateandtime','time','temp','month','count','rushhour','windspeed','humidity'], axis=1)
...
test_df = test_df.drop(['sunrise', 'sunset', 'date', 'dateandtime','time','temp','month','rushhour','windspeed','humidity'], axis=1)
#maybe convert time to an int

#create csv with new variables
train_df.to_csv('data/output-%s-%s.csv' % (data_time, trees))
train_df = train_df[['datetime','registered','casual','season','holiday','workingday','weather','atemp','hour','year','sunindicator','dow']]
